...
#     thread1 = Thread(target=rticonnector.writer())
#     thread2 = Thread(target=reader.reader())
#     thread1.start()
#     thread2.start()
...
def writer(connector):
        ...
        for i in range(1,5):
            ...
            output.instance.set_number("u", i*2)
            output.instance.set_number("shapesize", 30)
            output.instance.set_number("color", "BLUE")
            output.write()
            print(i)
...
if __name__ == "__main__":
    connector = rti.Connector(config_name="MyParticipantLibrary::MyPubParticipant",
